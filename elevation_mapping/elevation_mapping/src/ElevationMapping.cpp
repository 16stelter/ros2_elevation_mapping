...
namespace elevation_mapping
{
  ...
  ElevationMapping::ElevationMapping(rclcpp::Node &node)
      : node_(node), inputSources_(node_), map_(node_),
        robotMotionMapUpdater_(node_),
        receivedFirstMatchingPointcloudAndPose_(false)
  {
#ifndef NDEBUG
    // Print a warning if built in debug.
    RCLCPP_WARN(node_->get_logger(), "CMake Build Type is 'Debug'. Change to "
  ...
  { // Handle deprecated
    ...
    const bool hasDeprecatedPointcloudTopic =
        nodeHandle_.hasParam("point_cloud_topic");
    if (hasDeprecatedPointcloudTopic)
    {
      RCLCPP_WARN(node_->get_logger(), "Parameter 'point_cloud_topic' is deprecated, please use "
    ...
    if (!configuredInputSources && hasDeprecatedPointcloudTopic)
    {
      pointCloudSubscriber_ = nodeHandle_.subscribe<sensor_msgs::PointCloud2>(
          parameters.pointCloudTopic_, 1, [&](const auto &msg)
          { pointCloudCallback(msg, true, sensorProcessor_); });
    }
    if (configuredInputSources)
    {
      inputSources_.registerCallbacks(
          *this, make_pair("pointcloud", &ElevationMapping::pointCloudCallback));
    }
  ...
  void ElevationMapping::setupServices()
  {
    ...
    // Multi-threading for fusion. TODO!
    fusionTriggerService_ = node_->create_service<std_srvs : srv::Empty>(
        "trigger_fusion",
        std::bind(&ElevationMapping::fuseEntireMapServiceCallback, this,
                  std::placeholders::_1, std::placeholders::_2));
    fusedSubmapService_ = node_->create_service<grid_map_msgs::GetGridMap>(
        "get_submap", std::bind(&ElevationMapping::getFusedSubmapServiceCallback, this,
                                std::placeholders::_1, std::placeholders::_2));
    rawSubmapService_ = node_->create_service<grid_map_msgs::GetGridMap>(
        "get_raw_submap", std::bind(&ElevationMapping::getRawSubmapServiceCallback, this,
                                    std::placeholders::_1, std::placeholders::_2));
    ...
    enableUpdatesService_ = node_->create_service<std_srvs::srv::Empty>(
        "enable_updates", std::bind(&ElevationMapping::clearMapServiceCallback, this,
                                    std::placeholders::_1, std::placeholders::_2));
    disableUpdatesService_ = node_->create_service<std_srvs::srv::Empty>(
        "disable_updates", std::bind(&ElevationMapping::clearMapServiceCallback, this,
                                     std::placeholders::_1, std::placeholders::_2));
    maskedReplaceService_ = nodeHandle_.advertiseService(
        "masked_replace", &ElevationMapping::maskedReplaceServiceCallback, this);
    saveMapService_ = nodeHandle_.advertiseService(
        "save_map", &ElevationMapping::saveMapServiceCallback, this);
    loadMapService_ = nodeHandle_.advertiseService(
        "load_map", &ElevationMapping::loadMapServiceCallback, this);
    reloadParametersService_ = nodeHandle_.advertiseService(
        "reload_parameters", &ElevationMapping::reloadParametersServiceCallback,
        this);
  }
  ...
  void ElevationMapping::setupTimers()
  {
    ...
    mapUpdateTimer_ = node_->create_wall_timer(
        parameters.maxNoUpdateDuration_,
        std::bind(&ElevationMapping::mapUpdateTimerCallback, this), true);
    mapUpdateTimer_->cancel();

    if (!parameters.fusedMapPublishTimerDuration_.isZero())
    {
      fusedMapPublishTimer_ = node_->create_wall_timer(
          parameters_.fusedMapPublishTimerDuration_,
          rclcpp::TimerBase::Options()
              .set_callback([this]()
                            { publishFusedMapCallback(); })
              .use_executor(fusionServiceExecutor_));
    ...
    if (mapParameters.enableVisibilityCleanup_ &&
        !parameters.visibilityCleanupTimerDuration_.isZero() &&
        !mapParameters.enableContinuousCleanup_)
    {
      visibilityCleanupTimer_ = node_->create_wall_timer(
          parameters_.visibilityCleanupTimerDuration_,
          rclcpp::TimerBase::Options()
              .set_callback([this]()
                            { visibilityCleanupCallback(); })
              .use_executor(visibilityCleanupExecutor_));
  ...
  {
    // Using getDataToWrite gets a write-lock on the parameters thereby blocking
    // all reading threads for the whole scope of this readParameters, which is
    // desired.
    ...
    nodeHandle_.param("point_cloud_topic", parameters.pointCloudTopic_,
                      std::string("/points"));
    nodeHandle_.param("robot_pose_with_covariance_topic",
                      parameters.robotPoseTopic_, std::string("/pose"));
    nodeHandle_.param("track_point_frame_id", parameters.trackPointFrameId_,
                      std::string("/robot"));
    ...
    nodeHandle_.param("robot_pose_cache_size", parameters.robotPoseCacheSize_,
                      200);
    assert(parameters.robotPoseCacheSize_ >= 0);
    ...
    assert(!parameters.maxNoUpdateDuration_.isZero());
    ...
    nodeHandle_.param("fused_map_publishing_rate", fusedMapPublishingRate,
                      fusedMapPublishingRate);
    if (fusedMapPublishingRate == 0.0)
    {
      parameters.fusedMapPublishTimerDuration_.fromSec(0.0);
      RCLCPP_WARN(node_->get_logger(), "Rate for publishing the fused map is zero. The fused elevation "
                                       "map will not be published unless the service `triggerFusion` is "
                                       "called.");
    }
    ...
    {
      parameters.fusedMapPublishTimerDuration_.fromSec(1.0 /
                                                       fusedMapPublishingRate);
    }

    double visibilityCleanupRate{1.0};
    nodeHandle_.param("visibility_cleanup_rate", visibilityCleanupRate,
                      visibilityCleanupRate);
    ...
    {
      parameters.visibilityCleanupTimerDuration_.fromSec(1.0 /
                                                         visibilityCleanupRate);
    ...
    nodeHandle_.param("map_frame_id", parameters.mapFrameId_,
                      std::string("/map"));
    ...
    nodeHandle_.param("mahalanobis_distance_threshold",
                      mapParameters.mahalanobisDistanceThreshold_, 2.5);
    nodeHandle_.param("multi_height_noise", mapParameters.multiHeightNoise_,
                      pow(0.003, 2));
    nodeHandle_.param("min_horizontal_variance",
                      mapParameters.minHorizontalVariance_,
                      pow(resolution / 2.0, 2)); // two-sigma
    nodeHandle_.param("max_horizontal_variance",
                      mapParameters.maxHorizontalVariance_, 0.5);
    nodeHandle_.param("underlying_map_topic", mapParameters.underlyingMapTopic_,
                      std::string());
    nodeHandle_.param("enable_visibility_cleanup",
                      mapParameters.enableVisibilityCleanup_, true);
    nodeHandle_.param("enable_continuous_cleanup",
                      mapParameters.enableContinuousCleanup_, false);
    nodeHandle_.param("scanning_duration", mapParameters.scanningDuration_, 1.0);
    nodeHandle_.param("increase_height_alpha", mapParameters.increaseHeightAlpha_,
                      0.0);
    nodeHandle_.param("masked_replace_service_mask_layer_name",
                      parameters.maskedReplaceServiceMaskLayerName_,
                      std::string("mask"));

    // Settings for initializing elevation map
    nodeHandle_.param("initialize_elevation_map",
                      parameters.initializeElevationMap_, false);
    nodeHandle_.param("initialization_method", parameters.initializationMethod_,
                      0);
    nodeHandle_.param("length_in_x_init_submap", parameters.lengthInXInitSubmap_,
                      1.2);
    nodeHandle_.param("length_in_y_init_submap", parameters.lengthInYInitSubmap_,
                      1.8);
    nodeHandle_.param("init_submap_height_offset",
                      parameters.initSubmapHeightOffset_, 0.0);
    nodeHandle_.param("init_submap_variance", parameters.initSubmapVariance_,
                      0.01);
    nodeHandle_.param("target_frame_init_submap",
                      parameters.targetFrameInitSubmap_,
                      std::string("/footprint"));

    // SensorProcessor parameters. Deprecated, use the sensorProcessor from within
    // input sources instead!
    std::string sensorType;
    nodeHandle_.param("sensor_processor/type", sensorType,
                      std::string("structured_light"));

    SensorProcessorBase::GeneralParameters generalSensorProcessorConfig{
        nodeHandle_.param("robot_base_frame_id", std::string("/robot")),
        parameters.mapFrameId_};
    if (sensorType == "structured_light")
    {
      sensorProcessor_ = std::make_unique<StructuredLightSensorProcessor>(
          nodeHandle_, generalSensorProcessorConfig);
    }
    else if (sensorType == "stereo")
    {
      sensorProcessor_ = std::make_unique<StereoSensorProcessor>(
          nodeHandle_, generalSensorProcessorConfig);
    }
    else if (sensorType == "laser")
    {
      sensorProcessor_ = std::make_unique<LaserSensorProcessor>(
          nodeHandle_, generalSensorProcessorConfig);
    }
    else if (sensorType == "perfect")
    {
      sensorProcessor_ = std::make_unique<PerfectSensorProcessor>(
          nodeHandle_, generalSensorProcessorConfig);
    }
    else
    {
  ...
  bool ElevationMapping::initialize()
  {
    ...
    fusionServiceThread_ =
        boost::thread(&ElevationMapping::runFusionServiceThread, this);
      ...
      bool publishPointCloud, const SensorProcessorBase::Ptr &sensorProcessor_)
  {
    ...
    {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), 10, "Updating of elevation map is disabled. (Warning "
    ...
    if (!receivedFirstMatchingPointcloudAndPose_)
    {
      ...
      {
        RCLCPP_WARN_THROTTLE(node_->get_logger(), 5, "No corresponding point cloud and pose are found. Waiting for "
                                                     "first match. (Warning message is throttled, 5s.)");
        return;
      }
      else
      {
        RCLCPP_INFO(node_->get_logger(), "First corresponding point cloud and pose found, elevation "
                                         "mapping started. ");
    ...
    if (!parameters.ignoreRobotMotionUpdates_)
    {
      ...
      if (!poseMessage)
      {
        ...
        if (robotPoseCache_.getOldestTime().toSec() >
            lastPointCloudUpdateTime_.toSec())
        {
          RCLCPP_ERROR(node_->get_logger(), "The oldest pose available is at %f, requested pose at %f",
                       robotPoseCache_.getOldestTime().toSec(),
                       lastPointCloudUpdateTime_.toSec());
        }
        else
        {
          RCLCPP_ERROR(node_->get_logger(), "Could not get pose information from robot for time %f. "
                                            "Buffer empty?",
                       lastPointCloudUpdateTime_.toSec());
        }
        return;
                                   ...
                                   pointCloudMsg->header.frame_id))
    {
      ...
      {
        RCLCPP_INFO_THROTTLE(node_->get_logger(), 10, "Waiting for tf transformation to be available. "
                                                      "(Message is throttled, 10s.)");
        return;
      }
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), 10,
                            "Point cloud could not be processed. (Throttled 10s)");
      resetMapUpdateTimer();
      return;
    }
  ...
  void ElevationMapping::mapUpdateTimerCallback()
  {
    ...
    if ((lastPointCloudUpdateTime_ - time) <=
        parameters.maxNoUpdateDuration_)
    { // there were updates from sensordata,
      // no need to force an update.
      return;
    }
    RCLCPP_WARN_THROTTLE(node_->get_logger(), 5, "Elevation map is updated without data from the sensor. "
  ...
  bool ElevationMapping::updatePrediction(const rclcpp::Time &time)
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(), "Updating map with latest prediction from time %f.",
    ...
    {
      RCLCPP_DEBUG(node_->get_logger(), "Requested update with time stamp %f, but time of last update "
    ...
    boost::shared_ptr<geometry_msgs::PoseWithCovarianceStamped const>
        poseMessage = robotPoseCache_.getElemBeforeTime(time);
    if (!poseMessage)
    {
      ...
      {
        RCLCPP_ERROR(node_->get_logger(), "Could not get pose information from robot for time %f. Buffer "
                  "empty?",
                  lastPointCloudUpdateTime_.toSec());
      }
      return false;
  ...
  bool ElevationMapping::updateMapLocation()
  {
    ...
    try
    {
      transformListener_.transformPoint(map_.getFrameId(), trackPoint,
      ...
      grid_map_msgs::GetGridMap::Response &response)
  {
    ...
    RCLCPP_DEBUG(node_->get_logger(), "Elevation submap request: Position x=%f, y=%f, Length x=%f, y=%f.",
    ...
    grid_map::GridMap subMap = map_.getFusedGridMap().getSubmap(
        requestedSubmapPosition, requestedSubmapLength, index, isSuccess);
    scopedLock.unlock();
    ...
    RCLCPP_DEBUG(node_->get_logger(), "Elevation submap responded with timestamp %f.",
  ...
  bool ElevationMapping::initializeElevationMap()
  {
    ...
    if (parameters.initializeElevationMap_)
    {
      if (static_cast<elevation_mapping::InitializationMethods>(
              parameters.initializationMethod_) ==
          elevation_mapping::InitializationMethods::PlanarFloorInitializer)
      {
        ...
        try
        {
          transformListener_.waitForTransform(parameters.mapFrameId_,
                                              parameters.targetFrameInitSubmap_,
                                              rclcpp::Time(0), rclcpp::Duration(5.0));
          transformListener_.lookupTransform(parameters.mapFrameId_,
                                             parameters.targetFrameInitSubmap_,
                                             rclcpp::Time(0), transform);
          RCLCPP_DEBUG_STREAM(node_->get_logger(), "Initializing with x: "
          ...
          map_.setRawSubmapHeight(
              positionRobot,
              transform.getOrigin().z() + parameters.initSubmapHeightOffset_,
              parameters.initSubmapVariance_, parameters.lengthInXInitSubmap_,
        ...
        catch (tf::TransformException &ex)
        {
          ...
          RCLCPP_WARN(node_->get_logger(), "Could not initialize elevation map with constant height. "
                   "(This warning can be ignored if TF tree is not available.)");
          return false;
        }
      }
    }
  ...
  bool ElevationMapping::maskedReplaceServiceCallback(
      grid_map_msgs::SetGridMap::Request &request,
      grid_map_msgs::SetGridMap::Response & /*response*/)
  {
         ...
         sourceLayerIterator++)
    {
      ...
      {
        RCLCPP_ERROR(node_->get_logger(), "Masked replace service: Layer %s does not exist!",
  ...
  bool ElevationMapping::reloadParametersServiceCallback(
      ...
      std_srvs::Trigger::Response &response)
  {
    ...
    else
    {
      ...
      response.message =
          "Reloading parameters failed, reverted parameters to previous state!";
    }
  ...
  void ElevationMapping::resetMapUpdateTimer()
  {
    ...
    rclcpp::Duration periodSinceLastUpdate =
        rclcpp::Time::now() - map_.getTimeOfLastUpdate();
    if (periodSinceLastUpdate > parameters.maxNoUpdateDuration_)
    {
      periodSinceLastUpdate.fromSec(0.0);
    }
    mapUpdateTimer_.setPeriod(parameters.maxNoUpdateDuration_ -
                              periodSinceLastUpdate);
