...
ElevationMap::ElevationMap(rclcpp::Node node)
  ...
  node_->declare_parameter("postprocessor_num_threads", postprocessor_num_threads);
  node_->get_parameter("postprocessor_num_threads", postprocessor_num_threads);
  postprocessorPool = PostprocessorPool(postprocesor_num_threads, node_);
  ...
  if (!parameters.underlyingMapTopic_.empty()) {
    underlyingMapSubscriber_ = node_->create_subscription<grid_map_msgs::msg::GridMap>(parameters.underlyingMapTopic_, 1, 
  ...
  visibilityCleanupMapPublisher_ = node_->create_publisher<grid_map_msgs::GridMap>("visibility_cleanup_map", 1);
...
bool ElevationMap::add(const PointCloudType::Ptr pointCloud, Eigen::VectorXf& pointCloudVariances, const ros::Time& timestamp,
  ...
  const rclcpp::Time methodStartTime = rclcpp::Time::now();
  ...
  const rclcpp::Duration duration = rclcpp::Time::now() - methodStartTime;
...
void ElevationMap::visibilityCleanup(const rclcpp::Time& updatedTime) {
  ...
  rclcpp::Duration duration = rclcpp::Time::now() - methodStartTime;
